...
class Gauge extends Component {
    ...
    render () {
        ...
        const threshold = 25;
        let colors = {
            0: 'pear-m-fill-level-4',
            1: 'pear-m-fill-level-3',
            2: 'pear-m-fill-level-2',
            3: 'pear-m-fill-level-1',
            4: 'pear-m-fill-level-1'
        };

        const gaugeClasses = classNames(
            this.props.className,
            this.props.flipFullColors ? 'pear-m-negative' : '',
            'pear-c-gauge',
            colors[Math.floor(this.props.value / threshold)]
        );

        return (
            <div id={ this.props.identifier } className={ gaugeClasses }  widget-type='PearGauge' widget-id={ this.props.identifier } ></div>
        );
    }
}
