...
class Donut extends Component {
    ...
    updateLabels(donut) {
        if (this.props.withLegend) {
            select(this.legend)
            .selectAll('div.pear-l-donut__legend--item')
            .each(function() {
                select(this)
    ...
    render () {
        ...
        if (this.props.withLegend) {
            donutLegend =
            <div className='pear-l-donut__legend' ref={ ref => {this.legend = ref;} }>
                { this.props.values && this.props.values.map(oneItem => (
                    <div key={ oneItem }  data-id={ oneItem[0] } className="donut pear-l-donut__legend--item">
        ...
        return (
            <div className={ wrapperClasses } widget-type='PearDonut' widget-id={ this.props.identifier }>
                <div className='pear-l-donut'>
                    <div id={ this.props.identifier } className={ donutClasses }></div>
                    <div className='pear-c-donut-hole'>
                        <span className='pear-c-donut-hole--total__number'>{ total }</span>
                        <span className='pear-c-donut-hole--total__label'>{ this.props.totalLabel }</span>
                    </div>
                </div>
                { donutLegend }
            </div>
